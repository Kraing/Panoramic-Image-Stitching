...
public:
  ...
  static cv::Mat cylindricalProj_color(const cv::Mat& image,const double angle)
  {
      cv::Mat tmp, result;
      //cv::cvtColor(image, tmp, cv::COLOR_BGR2GRAY);
      tmp = image;
      ...
      for (int x = -half_width_image + 1, x_end = half_width_image; x < x_end; ++x)
      {
          for (int y = -half_height_image + 1, y_end = half_height_image; y < y_end; ++y)
          {
              ...
              if (x1 < half_width_image && x1 > -half_width_image + 1 && y1 < half_height_image && y1 > -half_height_image + 1)
              {
                  result.at<Vec3b>(y + half_height_image, x + half_width_image) = tmp.at<Vec3b>(round(y1 + half_height_image), round(x1 + half_width_image));
              }
          }
      }
      return result;
